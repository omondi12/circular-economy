...
<body class="bg-canvas text-ink antialiased font-sans min-h-screen flex flex-col">
    ...
    <header class="sticky top-0 z-40 bg-panel/90 backdrop-blur border-b border-border" x-data="{ mobileOpen: false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <nav class="hidden lg:flex items-center gap-1">
                    ...
                    <a href="{{ route('state-corporations.index') }}" @class(['px-3 py-2 rounded-lg text-sm font-medium transition-colors', 'bg-brand-50 text-brand-800' => request()->routeIs('state-corporations.*'), 'text-ink-muted hover:text-ink hover:bg-panel-muted' => ! request()->routeIs('state-corporations.*')])>{{ __('State Corporations') }}</a>
        ...
        <div x-show="mobileOpen" x-cloak x-transition class="lg:hidden border-t border-border bg-panel px-4 py-3 space-y-1">
            ...
            <a href="{{ route('state-corporations.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-ink-muted hover:bg-panel-muted">{{ __('State Corporations') }}</a>
